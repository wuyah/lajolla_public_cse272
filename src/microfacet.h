...
inline Real fresnel_dielectric(Real n_dot_i, Real n_dot_t, Real eta) {
    ...
    // if( !(n_dot_i >= 0 || n_dot_t >= 0 || eta > 0 )) {
    //     std::cout << n_dot_t  << " " <<n_dot_i <<std::endl;
    //     // exit(-1);
    //     return 1;
    // }
...
inline Vector3 sample_visible_normals_ani(const Vector3 &local_dir_in, Vector2 alpha, const Vector2 &rnd_param) {
    // The incoming direction is in the "ellipsodial configuration" in Heitz's paper
    if (local_dir_in.z < 0) {
        // Ensure the input is on top of the surface.
        return -sample_visible_normals_ani(-local_dir_in, alpha, rnd_param);
    }
